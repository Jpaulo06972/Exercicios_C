...
double evaluateExpression(Node* currentNode) {
    // 1. Se o nó é nulo, retornar 0.
    if (currentNode == NULL) {
        return 0.0;
    }

    // 2. Se for nó folha (operando, marcado com '#'), retornar currentNode->value.
    // Na sua implementação, os nós numéricos são criados com data = '#'.
    if (currentNode->data == '#') {
        return currentNode->value;
    }
    
    // Se não é um nó folha, é um operador.
    // 3. Calcular recursivamente o valor das subárvores esquerda e direita
    double esquerda = evaluateExpression(currentNode->left);
    double direita = evaluateExpression(currentNode->right);

    // Aplicar o operador currentNode->data sobre esses dois valores
    switch (currentNode->data) {
        ...
        case '*':
            return esquerda * direita;
        case '/':
            // Adicionar verificação de divisão por zero
            if (direita == 0.0) {
            ...
            return esquerda / direita;
        default:
            return 0.0; // Operador desconhecido
    }
}
